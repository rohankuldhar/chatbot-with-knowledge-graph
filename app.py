...
def main():
    load_dotenv()
    openai.api_key = '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'
        ...
        if query := st.chat_input("Ask questions?"):
            ...
            with st.chat_message("assistant"):
                    ...
                    messages=[
                        {
                        "role": "system",
                        "content": f"check if the '{result}' is  correct according to the '{query}' and if its correct, then respond with '{result}'  and if it's not correct just return 'Not in the pdf' "
                        }
                    ],
                    temperature=0,
                    max_tokens=256,
